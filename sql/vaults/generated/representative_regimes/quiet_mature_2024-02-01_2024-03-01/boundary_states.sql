-- Phase 1E-B authoritative boundary states for Quiet mature market.
WITH
...
balances AS (
    SELECT ilk, urn,
           SUM(CASE WHEN block_time_utc < TIMESTAMP '2024-02-01 00:00:00 UTC'
           ...
           COUNT_IF(block_time_utc < TIMESTAMP '2024-02-01 00:00:00 UTC')
               AS pre_window_mutation_count,
           COUNT_IF(block_time_utc >= TIMESTAMP '2024-02-01 00:00:00 UTC')
               AS window_mutation_count,
           MAX(CASE WHEN block_time_utc < TIMESTAMP '2024-02-01 00:00:00 UTC'
...
opening_rates AS (
    ...
    FROM (
        ...
        WHERE effective_time_utc < TIMESTAMP '2024-02-01 00:00:00 UTC'
    )
    WHERE rank = 1
),
end_rates AS (
