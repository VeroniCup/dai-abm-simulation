-- Phase 1E-B authoritative boundary states for Terra and CeFi contagion.
WITH
...
mutations AS (
    ...
    WHERE g.call_block_date >= DATE '2019-11-01'
      AND g.call_block_date < DATE '2022-06-20'
      AND g.call_block_time >= TIMESTAMP '2019-11-01 00:00:00 UTC'
      AND g.call_block_time < TIMESTAMP '2022-06-20 00:00:00 UTC'
      AND g.contract_address = 0x35d1b3f3d7966a1dfe207aa4514c12a259a0492b
      AND g.call_success = true
),
balances AS (
    SELECT ilk, urn,
           SUM(CASE WHEN block_time_utc < TIMESTAMP '2022-05-05 00:00:00 UTC'
           ...
           COUNT_IF(block_time_utc < TIMESTAMP '2022-05-05 00:00:00 UTC')
               AS pre_window_mutation_count,
           COUNT_IF(block_time_utc >= TIMESTAMP '2022-05-05 00:00:00 UTC')
               AS window_mutation_count,
           MAX(CASE WHEN block_time_utc < TIMESTAMP '2022-05-05 00:00:00 UTC'
...
opening_rates AS (
    ...
    FROM (
        ...
        WHERE effective_time_utc < TIMESTAMP '2022-05-05 00:00:00 UTC'
    )
    WHERE rank = 1
),
end_rates AS (
