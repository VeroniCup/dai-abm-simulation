...
transactions AS (
    ...
    WHERE block_date >= DATE '{{START_DATE}}'
      ...
      AND block_time >= TIMESTAMP '{{START_TIMESTAMP}}'
      AND block_time < TIMESTAMP '{{END_TIMESTAMP}}'
),
mutations AS (
    SELECT
    ...
    WHERE f.call_block_date >= DATE '{{START_DATE}}'
      AND f.call_block_date < DATE '{{END_DATE}}'
      AND f.call_block_time >= TIMESTAMP '{{START_TIMESTAMP}}'
      AND f.call_block_time < TIMESTAMP '{{END_TIMESTAMP}}'
    ...
    WHERE g.call_block_date >= DATE '{{START_DATE}}'
      AND g.call_block_date < DATE '{{END_DATE}}'
      AND g.call_block_time >= TIMESTAMP '{{START_TIMESTAMP}}'
      AND g.call_block_time < TIMESTAMP '{{END_TIMESTAMP}}'
